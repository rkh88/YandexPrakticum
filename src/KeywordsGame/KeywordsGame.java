...
public class KeywordsGame {
    ...
    public void runGame() {
        final int number = random.nextInt(KEYWORDS.size());
        int incorrectAnswersCounter = 0;
        while (true) {
            final boolean providedAnswer;
            if (number < RESERVED.size()) {
    ...
    private boolean writeReservedQuestion() {
        ...
        final int num = random.nextInt(RESERVED.size());
        System.out.println(RESERVED.get(num));
        return true;
    }
